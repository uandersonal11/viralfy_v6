...
export default function RoteiroGenerator() {
  ...
  const handleToneSelection = (toneValue: string) => {
    setTones(prev => {
      ...
      notifications.info({
        title: prev.includes(toneValue) ? 'Tom removido' : 'Tom adicionado',
        description: prev.includes(toneValue) 
          ? `Tom "${toneOptions.find(t => t.value === toneValue)?.label}" removido`
          : `Tom "${toneOptions.find(t => t.value === toneValue)?.label}" adicionado`,
        icon: Check,
      })
